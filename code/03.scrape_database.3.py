...
ARCHIVE_URL = "https://www.aup-online.com"

def get_list_of_volumes(base_url):
    """Fetches list of available volumes from the archive page."""
    ...
    return [
        (a_tag.get_text(strip=True).replace('\r', ''), ARCHIVE_URL + a_tag['href'].split('&showDates')[0])
        for item in volume_items if (a_tag := item.find('a'))
    ]

def get_list_of_issues(volume_items):
    """Extracts individual issue details from volume pages."""
    results = []
    for volume, volume_url in volume_items:
        ...
        for issue in soup.find_all('li'):
            ...
            if issue_name and issue_month:
                results.append((volume, volume_url, issue_url, issue_name.get_text(strip=True), issue_month.get_text(strip=True).split('\n')[1]))
    
    return results

def extract_article_info(article):
    """Extracts metadata from an article element."""
    ...
    section = 'boekbespreking' if title and 'Boekbespreking' in title else 'algemeen'
    
    info_tag = article.find('div', class_='js-desc')
    eng_title, abstract = None, None
    if info_tag:
        info_parts = [p.get_text(strip=True).replace(' .', '.') for p in info_tag.find_all('p')]
        if info_parts:
            abstract = info_parts[-1]
            eng_title = ' -- '.join(info_parts[:-1]) + '.' if len(info_parts) > 1 else None
    
    authors = ', '.join(a.get_text(strip=True) for a in article.find_all('a', class_='nonDisambigAuthorLink')) if article.find_all('a', class_='nonDisambigAuthorLink') else None
    
    return title, eng_title, doi_url, authors, section, abstract

def get_article_data(issue_items):
    """Fetches article metadata from issue pages."""
    results = []
    ...
    for volume, _, issue_url, issue_name, issue_month in issue_items:
        ...
        article_items = soup.select_one("#main-content-container div.issue-listing div.panel-body div.publistwrapper")
        if not article_items:
            continue
        
        for article in article_items.find_all('ul', class_='list-unstyled', role='listitem'):
...
def add_pdf_url(df):
    """Adds PDF download URLs to the DataFrame."""
    results = []
    
    for _, row in tqdm(df.iterrows(), total=df.shape[0]):
        response = requests.get(row['doi_url'])
        soup = BeautifulSoup(response.content, 'html.parser')
        form = soup.find('form', class_='ft-download-content__form ft-download-content__form--pdf js-ft-download-form')
        
        pdf_url = requests.compat.urljoin(ARCHIVE_URL, form.get('action')) if form else None
        results.append({'id': row['id'], 'pdf_url': pdf_url})
    
    return df.merge(pd.DataFrame(results), how='inner')

def format_issue(df):
    """Formats the issue column to include volume numbers."""
    issue_fix = (
        df['volume']
        ...
        .assign(
            issue = lambda x: "Vol " + x['volume'] + " No " + x['issue'] + " (" + x['year'] + ')'
        )
        .drop('temp', axis=1)
    )
